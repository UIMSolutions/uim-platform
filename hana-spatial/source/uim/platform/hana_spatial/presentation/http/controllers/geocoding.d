...
class GeocodingController : ManageHttpController {
  protected ManageGeocodingResultsUseCase usecase;
  ...
  protected Json geocodeHandler(HTTPServerRequest req) {
    ...
    return successResponse("Geocoding result stored successfully", "Stored", 201, Json.emptyObject
        .set("id", result.id));
  }

  mixin(HandleTemplate!("handleGeocode", "geocodeHandler"));

  protected Json reverseGeocodeHandler(HTTPServerRequest req) {
    auto precheck = super.postHandler(req);
    if (precheck.hasError)
      return precheck;
    ...
    r.tenantId = tenantId;
    r.id = precheck.id;
    r.latitude = jsonDouble(data, "latitude");
    r.longitude = jsonDouble(data, "longitude");
    r.language = data.getString("language");
    r.providerId = data.getString("providerId");
    ...
    return successResponse("Reverse geocoding result stored successfully", "Stored", 201, Json.emptyObject
        .set("id", result.id));
  }

  mixin(HandleTemplate!("handleReverseGeocode", "reverseGeocodeHandler"));
