...
class BusinessController {
    ...
    void getBusiness(HTTPServerRequest req, HTTPServerResponse res, string id) {
        if (!req.session || !req.session.get!bool("isLoggedIn", false)) {
            res.redirect("/web/login");
            return;
        }
        ...
        auto block = BusinessBlock();
        res.render!("business_detail.dt", username, lang, translations, block);
    }
}