module uim.platform.architecture.presentation.ui5.views.architecture_blocks;
...
class ArchitectureBlockUi5View {
    string renderHub(string tenantId) {
        ...
        html ~= "var page=new sap.m.Page({title:'Architecture Architecture Blocks (UI5)'});";
    ...
    string renderOverview(Ui5ArchitectureBlockPageModel model) {
        auto html = appender!string();
    ...
    string renderDetail(Ui5ArchitectureBlockDetailModel model) {
        auto html = appender!string();
        auto tenant = escapeJs(model.tenantId);
        html ~= pageStart(model.title);
        html ~= "<script>";
        ...
        html ~= "if(!data.found){page.addContent(new sap.m.MessageStrip({text:'Architecture block not found',type:'Warning',showIcon:true}));}";
        html ~= "else{";
