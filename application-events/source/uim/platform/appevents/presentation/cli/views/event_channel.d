...
class CliEventChannelView {
    ...
    void renderDetail(EventChannel i) {
        writeln("=== Event Channel Detail ===");
        writefln("  ID            : %s", i.id.value);
        writefln("  Name          : %s", i.name);
        writefln("  Topic ID      : %s", i.topicId.value);
        writefln("  Channel Type  : %s", i.channelType);
        writefln("  Endpoint      : %s", i.endpoint);
        writefln("  Status        : %s", i.status.to!string);
        writefln("  Delivery Mode : %s", i.deliveryMode);
        writefln("  Max Size Bytes: %d", i.maxSizeBytes);
    }
