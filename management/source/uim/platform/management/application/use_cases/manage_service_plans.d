...
class ManageServicePlansUseCase
{
    ...
    CommandResult update(ServicePlanId id, UpdateServicePlanRequest req)
    {
        ...
        if (req.planDisplayName.length > 0) plan.planDisplayName = req.planDisplayName;
        if (req.description.length > 0) plan.description = req.description;
        if (req.availableRegions.length > 0) plan.availableRegions = req.availableRegions;
        if (req.maxQuota > 0) plan.maxQuota = req.maxQuota;
        plan.isBeta = req.isBeta;
        plan.provisionable = req.provisionable;
        if (req.metadata.length > 0) plan.metadata = req.metadata;
    ...
    ServicePlan getById(ServicePlanId id) { return repo.findById(id); }
    ServicePlan[] listAll() { return repo.findAll(); }
    ServicePlan[] listByService(string serviceName) { return repo.findByService(serviceName); }
    ServicePlan[] listByCategory(string category) { return repo.findByCategory(parseCategory(category)); }
    ServicePlan[] listByRegion(string region) { return repo.findByRegion(region); }
    ...
    private ServicePlanCategory parseCategory(string s)
    {
        switch (s)
        {
            ...
            default: return ServicePlanCategory.service;
        }
    }
