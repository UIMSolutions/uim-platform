...
class ManageReplicationJobsUseCase
{
    ...
    this(ReplicationJobRepository repo) { this.repo = repo; }
    ...
    CommandResult completeJob(ReplicationJobId id, long successRecords, long errorRecords, long skippedRecords, string[] errorMessages, string deltaToken)
    {
        auto job = repo.findById(id);
        if (job.id.length == 0)
            return CommandResult(false, "", "Replication job not found");
    ...
    ReplicationJob getJob(ReplicationJobId id) { return repo.findById(id); }
    ReplicationJob[] listByTenant(TenantId tenantId) { return repo.findByTenant(tenantId); }
    ReplicationJob[] listByStatus(TenantId tenantId, string status) { return repo.findByStatus(tenantId, parseJobStatus(status)); }
    ReplicationJob[] listByDistributionModel(TenantId tenantId, DistributionModelId modelId) { return repo.findByDistributionModel(tenantId, modelId); }
    ...
    private ReplicationTrigger parseTrigger(string s)
    {
        switch (s)
        {
            case "manual": return ReplicationTrigger.manual;
            case "scheduled": return ReplicationTrigger.scheduled;
            case "eventDriven": return ReplicationTrigger.eventDriven;
            case "onChange": return ReplicationTrigger.onChange;
            default: return ReplicationTrigger.manual;
        }
    }
    ...
    private ReplicationJobStatus parseJobStatus(string s)
    {
        switch (s)
        {
            case "pending": return ReplicationJobStatus.pending;
            case "running": return ReplicationJobStatus.running;
            case "completed": return ReplicationJobStatus.completed;
            case "failed": return ReplicationJobStatus.failed;
            case "cancelled": return ReplicationJobStatus.cancelled;
            case "paused": return ReplicationJobStatus.paused;
            default: return ReplicationJobStatus.pending;
        }
    }
