module uim.platform.master_data_integration.application.usecases.manage_data_models;
...
class ManageDataModelsUseCase
{
    ...
    this(DataModelRepository repo) { this.repo = repo; }
    ...
    CommandResult updateModel(DataModelId id, UpdateDataModelRequest req)
    {
        ...
        if (req.version_.length > 0) model.version_ = req.version_;
        if (req.fields.length > 0) model.fields = toFieldDefs(req.fields);
        if (req.keyFields.length > 0) model.keyFields = req.keyFields;
        if (req.requiredFields.length > 0) model.requiredFields = req.requiredFields;
    ...
    DataModel getModel(DataModelId id) { return repo.findById(id); }
    DataModel[] listByTenant(TenantId tenantId) { return repo.findByTenant(tenantId); }
    DataModel[] listByCategory(TenantId tenantId, string category) { return repo.findByCategory(tenantId, parseCategory(category)); }
    DataModel findByName(TenantId tenantId, string name) { return repo.findByName(tenantId, name); }
    ...
    private FieldType parseFieldType(string s)
    {
        switch (s)
        {
            case "string": return FieldType.string_;
            case "integer": return FieldType.integer_;
            case "decimal": return FieldType.decimal_;
            ...
            case "array": return FieldType.array_;
            case "object": return FieldType.object_;
            default: return FieldType.string_;
        }
    }
