module uim.platform.data.privacy.application.usecases.manage_legal_grounds;
...
class ManageLegalGroundsUseCase
{
    ...
    CommandResult updateGround(UpdateLegalGroundRequest req)
    {
        ...
        if (req.description.length > 0) ground.description = req.description;
        if (req.legalReference.length > 0) ground.legalReference = req.legalReference;
        if (req.categories.length > 0) ground.categories = req.categories;
        ground.isActive = req.isActive;
        if (req.validUntil > 0) ground.validUntil = req.validUntil;
