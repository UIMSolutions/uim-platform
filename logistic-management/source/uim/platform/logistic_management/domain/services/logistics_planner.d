...
public:
  ...
  bool canTransitionDelivery(DeliveryStatus current, DeliveryStatus next) {
    ...
    case DeliveryStatus.created:
      return next == DeliveryStatus.picking;
    case DeliveryStatus.picking:
      return next == DeliveryStatus.packed || next == DeliveryStatus.cancelled;
    case DeliveryStatus.packed:
      return next == DeliveryStatus.shipped || next == DeliveryStatus.cancelled;
  ...
  bool canTransitionFreightOrder(FreightOrderStatus current, FreightOrderStatus next) {
    final switch (current) {
    case FreightOrderStatus.draft:
      return next == FreightOrderStatus.planned || next == FreightOrderStatus.cancelled;
    case FreightOrderStatus.planned:
      return next == FreightOrderStatus.inTransit || next == FreightOrderStatus.cancelled;
    case FreightOrderStatus.inTransit:
      return next == FreightOrderStatus.delivered;
    case FreightOrderStatus.delivered:
      return false;
    case FreightOrderStatus.cancelled:
      return false;
    }
  }
  ...
  bool canTransitionTask(WarehouseTaskStatus current, WarehouseTaskStatus next) {
    final switch (current) {
    case WarehouseTaskStatus.created:
      return next == WarehouseTaskStatus.queued || next == WarehouseTaskStatus.cancelled;
    case WarehouseTaskStatus.queued:
      return next == WarehouseTaskStatus.inProgress || next == WarehouseTaskStatus.cancelled;
    case WarehouseTaskStatus.inProgress:
      return next == WarehouseTaskStatus.confirmed || next == WarehouseTaskStatus.cancelled;
    case WarehouseTaskStatus.confirmed:
      return false;
