...
@safe:
class ShipmentRepository : TenantRepository!(Shipment, ShipmentId), ShipmentRepository {
  override Shipment[] findByFreightOrder(TenantId tenantId, FreightOrderId freightOrderId) {
    return findByTenant(tenantId).filter!(s => s.freightOrderId.value == freightOrderId.value).array;
  }
  ...
  override Shipment[] findByDirection(TenantId tenantId, LogisticsDirection direction) {
    return findByTenant(tenantId).filter!(s => s.direction == direction).array;
  }

  override Shipment[] findByPartner(TenantId tenantId, string partnerId) {
    return findByTenant(tenantId).filter!(s => s.partnerId == partnerId).array;
  }
