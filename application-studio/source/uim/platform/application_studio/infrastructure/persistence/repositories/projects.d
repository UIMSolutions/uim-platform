...
unittest {
    ...
    void testFindByType(IProjectRepository repo) {
        auto tenantId = TenantId("tenant1");
        auto projectType = "sapFiori".toProjectType;
        auto project1 = Project(tenantId, ProjectId("project1"));
        project1.projectType = projectType;
        ...
        auto projectsOfType = repo.findByType(tenantId, projectType);
        assert(projectsOfType.length == 2);
        assert(projectsOfType.canFind(project1));
        assert(projectsOfType.canFind(project2));
        ...
        assert(repo.findByType(tenantId, projectType).length == 0);
    }
    ...
    void testCountByDevSpace(IProjectRepository repo) {
        auto tenantId = TenantId("tenant1");
        ...
        // Count projects by dev space
        auto countDevSpace1 = repo.countByDevSpace(tenantId, devSpaceId);
        assert(countDevSpace1 == 2);

        auto countDevSpace2 = repo.countByDevSpace(tenantId, DevSpaceId("devspace2"));
        assert(countDevSpace2 == 1);
