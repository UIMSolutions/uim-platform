module application.use_cases.manage_workflows;
...
class ManageWorkflowsUseCase
{
    ...
    CommandResult createWorkflow(CreateWorkflowRequest req)
    {
        ...
        foreach (ref tmpl; scenario.stepTemplates)
        {
            ...
            if (tmpl.requiresSourceSystem) step.sourceSystemId = req.sourceSystemId;
            if (tmpl.requiresTargetSystem) step.targetSystemId = req.targetSystemId;
