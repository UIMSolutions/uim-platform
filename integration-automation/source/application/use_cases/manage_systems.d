module application.use_cases.manage_systems;
...
class ManageSystemsUseCase
{
    ...
    CommandResult updateSystem(UpdateSystemRequest req)
    {
        ...
        if (req.host.length > 0) updated.host = req.host;
        if (req.port > 0) updated.port = req.port;
        if (req.client.length > 0) updated.client = req.client;
        if (req.protocol.length > 0) updated.protocol = req.protocol;
        updated.status = req.status;
        if (req.environment.length > 0) updated.environment = req.environment;
        if (req.region.length > 0) updated.region = req.region;
        if (req.systemId.length > 0) updated.systemId = req.systemId;
        if (req.tenant.length > 0) updated.tenant = req.tenant;
