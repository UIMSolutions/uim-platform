module application.use_cases.manage_scenarios;
...
class ManageScenariosUseCase
{
    ...
    CommandResult updateScenario(UpdateScenarioRequest req)
    {
        ...
        if (req.version_.length > 0) updated.version_ = req.version_;
        updated.status = req.status;
        updated.sourceSystemType = req.sourceSystemType;
        updated.targetSystemType = req.targetSystemType;
        if (req.prerequisites.length > 0) updated.prerequisites = req.prerequisites;
        if (req.stepTemplates.length > 0) updated.stepTemplates = req.stepTemplates;
