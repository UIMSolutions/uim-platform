...
class ManageDestinationsUseCase
{
    ...
    CommandResult updateDestination(UpdateDestinationRequest req)
    {
        ...
        if (req.url.length > 0) updated.url = req.url;
        ...
        if (req.user.length > 0) updated.user = req.user;
        if (req.tokenServiceUrl.length > 0) updated.tokenServiceUrl = req.tokenServiceUrl;
        if (req.tokenServiceUser.length > 0) updated.tokenServiceUser = req.tokenServiceUser;
        if (req.audience.length > 0) updated.audience = req.audience;
        if (req.scope_.length > 0) updated.scope_ = req.scope_;
