...
class MemoryAppVersionRepository : AppVersionRepository
{
  ...
  {
    return store.byValue().filter!(e => e.appId == appId && e.platform == platform).array;
  }

  AppVersion findLatestReleased(MobileAppId appId, MobilePlatform platform)
  {
    auto released = store.byValue().filter!(e => e.appId == appId
        && e.platform == platform && e.status == VersionStatus.released).array;
