...
*****************************************************************************************************************/
module uim.platform.mobile.application.usecases.manage_push_notifications;
...
import uim.platform.mobile.domain.ports.push_template_repository;
import uim.platform.mobile.domain.ports.push_campaign_repository;
import uim.platform.mobile.domain.entities.push_template;
import uim.platform.mobile.domain.entities.push_campaign;
import uim.platform.mobile.domain.services.push_dispatcher;
...
class ManagePushNotificationsUseCase
{
  ...
  this(PushNotificationRepository notifRepo, PushTemplateRepository templateRepo,
      PushCampaignRepository campaignRepo, PushDispatcher dispatcher)
  {
  ...
  CommandResult send(SendPushRequest req)
  {
    ...
    if (req.deepLink.length > 0)
      n.deepLink = req.deepLink;
    ...
    if (n.status == PushStatus.sent)
      n.sentAt = clockSeconds();
  ...
  PushNotification getNotification(PushNotificationId id)
  {
  ...
  {
    return notifRepo.findByApp(appId);
  }
  ...
  PushTemplate getTemplate(PushTemplateId id)
  {
    return templateRepo.findById(id);
  }

  PushTemplate[] listTemplates(MobileAppId appId)
  {
    return templateRepo.findByApp(appId);
  }
  ...
  PushCampaign getCampaign(PushCampaignId id)
  {
    return campaignRepo.findById(id);
  }

  PushCampaign[] listCampaigns(MobileAppId appId)
  {
    return campaignRepo.findByApp(appId);
  }
