...
class ManageMobileAppsUseCase
{
  ...
  CommandResult update(MobileAppId id, UpdateMobileAppRequest req)
  {
    ...
    if (req.description.length > 0)
      app.description = req.description;
    if (req.iconUrl.length > 0)
      app.iconUrl = req.iconUrl;
    if (req.deepLinkScheme.length > 0)
      app.deepLinkScheme = req.deepLinkScheme;
    app.pushEnabled = req.pushEnabled;
    app.offlineEnabled = req.offlineEnabled;
    app.analyticsEnabled = req.analyticsEnabled;
    app.loggingEnabled = req.loggingEnabled;
    if (req.securityPolicyId.length > 0)
      app.securityPolicyId = req.securityPolicyId;
    if (req.backendUrl.length > 0)
      app.backendUrl = req.backendUrl;
    if (req.customSettings.length > 0)
      app.customSettings = req.customSettings;
    if (req.status.length > 0)
      app.status = parseAppStatus(req.status);
...
private MobileAuthType parseAuthType(string s)
{
  ...
  {
  case "saml":
    return MobileAuthType.saml;
  case "basicAuth":
    return MobileAuthType.basicAuth;
  case "certificateBased":
    return MobileAuthType.certificateBased;
  case "biometric":
    return MobileAuthType.biometric;
  case "apiKey":
    return MobileAuthType.apiKey;
  case "noAuth":
    return MobileAuthType.noAuth;
  default:
    return MobileAuthType.oauth2;
  }
}
