...
class ManageFeatureTogglesUseCase
{
    ...
    FeatureToggle getById(FeatureToggleId id) { return repo.findById(id); }
    FeatureToggle[] listByApp(MobileAppId appId) { return repo.findByApp(appId); }
...
private ToggleStatus parseToggleStatus(string s)
{
    switch (s)
    {
    case "enabled": return ToggleStatus.enabled;
    case "percentage": return ToggleStatus.percentage;
    case "userSegment": return ToggleStatus.userSegment;
    case "scheduled": return ToggleStatus.scheduled;
    default: return ToggleStatus.disabled;
    }
}
