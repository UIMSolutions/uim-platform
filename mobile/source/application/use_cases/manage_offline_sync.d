...
import domain.ports.sync_session_repository;
import domain.types;
...
class ManageOfflineSyncUseCase
{
    ...
    OfflineConfig getConfig(OfflineConfigId id) { return configRepo.findById(id); }
    OfflineConfig[] listConfigs(MobileAppId appId) { return configRepo.findByApp(appId); }
    ...
    CommandResult completeSync(SyncSessionId id, CompleteSyncSessionRequest req)
    {
        ...
        if (req.deltaToken.length > 0) s.deltaToken = req.deltaToken;
    ...
    SyncSession getSession(SyncSessionId id) { return sessionRepo.findById(id); }
    SyncSession[] listSessions(OfflineConfigId configId) { return sessionRepo.findByConfig(configId); }
    SyncSession[] listByDevice(string deviceId) { return sessionRepo.findByDevice(deviceId); }
}
...
private SyncStrategy parseSyncStrategy(string s)
{
    switch (s)
    {
    case "fullSync": return SyncStrategy.fullSync;
    case "onDemand": return SyncStrategy.onDemand;
    case "backgroundSync": return SyncStrategy.backgroundSync;
    default: return SyncStrategy.deltaSync;
    }
}
...
private ConflictResolution parseConflictResolution(string s)
{
    switch (s)
    {
    case "clientWins": return ConflictResolution.clientWins;
    case "serverWins": return ConflictResolution.serverWins;
    case "manual": return ConflictResolution.manual;
    case "merge": return ConflictResolution.merge;
    default: return ConflictResolution.lastWriteWins;
    }
}
...
private SyncSessionStatus parseSyncStatus(string s)
{
    switch (s)
    {
    case "failed": return SyncSessionStatus.failed;
    case "conflicted": return SyncSessionStatus.conflicted;
    case "cancelled": return SyncSessionStatus.cancelled;
    default: return SyncSessionStatus.completed;
    }
}
