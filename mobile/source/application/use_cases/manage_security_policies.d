...
class ManageSecurityPoliciesUseCase
{
  ...
  CommandResult create(CreateSecurityPolicyRequest req)
  {
    ...
    p.offlineAccessDurationHours = req.offlineAccessDurationHours > 0
      ? req.offlineAccessDurationHours : 168;
  ...
  SecurityPolicy getById(SecurityPolicyId id)
  {
    return repo.findById(id);
  }

  SecurityPolicy[] listByTenant(TenantId tenantId)
  {
...
private EnforcementLevel parseEnforcement(string s)
{
  ...
  {
  case "optional":
    return EnforcementLevel.optional;
  case "required":
    return EnforcementLevel.required;
  case "strict":
    return EnforcementLevel.strict;
  default:
    return EnforcementLevel.recommended;
  }
}
...
private MobileAuthType[] parseAuthTypes(string[] types)
{
  ...
  foreach (t; types)
  {
    ...
    {
    case "oauth2":
      result ~= MobileAuthType.oauth2;
      break;
    case "saml":
      result ~= MobileAuthType.saml;
      break;
    case "basicAuth":
      result ~= MobileAuthType.basicAuth;
      break;
    case "certificateBased":
      result ~= MobileAuthType.certificateBased;
      break;
    case "biometric":
      result ~= MobileAuthType.biometric;
      break;
    case "apiKey":
      result ~= MobileAuthType.apiKey;
      break;
    default:
      break;
    }
