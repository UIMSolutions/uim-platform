...
*****************************************************************************************************************/
module uim.platform.custom_domain.presentation.web.views.dashboard;
...
struct CustomDomainWebView {
    ...
    private string nav(string tenantId) const {
        auto html = appender!string();
        html.put("<header class=\"topbar\"><div><p class=\"eyebrow\">Custom Domain MVC</p>");
        ...
        html.put(navLink("Dashboard", "/web/custom-domain?tenantId=" ~ tenantId));
        html.put(navLink("Domains", "/web/custom-domain/domains?tenantId=" ~ tenantId));
        html.put(navLink("TLS configs", "/web/custom-domain/tls-configurations?tenantId=" ~ tenantId));
        html.put(navLink("Mappings", "/web/custom-domain/mappings?tenantId=" ~ tenantId));
        html.put(navLink("DNS records", "/web/custom-domain/dns-records?tenantId=" ~ tenantId));
        html.put(navLink("Private keys", "/web/custom-domain/private-keys?tenantId=" ~ tenantId));
        html.put(navLink("Certificates", "/web/custom-domain/certificates?tenantId=" ~ tenantId));
        html.put(navLink("Trusted certs", "/web/custom-domain/trusted-certificates?tenantId=" ~ tenantId));
        html.put("</nav>");
        return html.data;
    }

    private string navLink(string label, string href) const {
        return "<a class=\"nav-link\" href=\"" ~ escapeHtml(href) ~ "\">" ~ escapeHtml(label) ~ "</a>";
    }
    ...
    private string requestConsole(WebPageModel model) const {
        ...
        html.put("<form id=\"request-form\" class=\"request-form\"><label>Base URL<input id=\"base-url\" value=\"\"></label>");
    ...
    private string metricChip(WebMetricModel metric) const {
        return "<span class=\"metric\"><strong>" ~ escapeHtml(metric.value) ~ "</strong><em>" ~
            escapeHtml(metric.label) ~ "</em></span>";
    }
    ...
    private string script() const {
        return [    "<script>",
            ...
            "    const tenant = new URLSearchParams(window.location.search).get('tenantId') || 'default';",
            "    const preview = `X-Tenant-Id: ${tenant}\\n` +",
            "      `\\n${requestBody.value}`;",
    ...
    private string styles() const {
        return [    ":root {",
            ...
            ".eyebrow { margin: 0 0 8px; color: var(--accent); text-transform: uppercase; letter-spacing: 0.18em; font-size: 0.78rem; }",
