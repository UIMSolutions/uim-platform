...
class FeatureFlagController {
    ...
    private void handleCreate(HTTPServerRequest req, HTTPServerResponse res) @safe {
        ...
        dto.type_ = getString(data, "type", "BOOLEAN");
        dto.instanceId = getString(data, "instanceId");
        dto.defaultVariant = getString(data, "defaultVariant", "off");
        dto.labels = getStringMap(data, "labels");
        dto.createdBy = getString(data, "createdBy");

        auto varArr = data["variants"];
        if (varArr.isArray)
            foreach (v; varArr)
                dto.variants ~= parseVariantDTO(v);

        auto ruleArr = data["rules"];
        if (ruleArr.isArray)
            foreach (r; ruleArr)
                dto.rules ~= parseRuleDTO(r);
    ...
    private void handleGet(HTTPServerRequest req, HTTPServerResponse res) @safe {
        ...
        if (flag_.isNull) {
            writeError(res, 404, "Feature flag not found");
            return;
        }
    ...
    private void handleUpdate(HTTPServerRequest req, HTTPServerResponse res) @safe {
        ...
        dto.description = getString(data, "description", "");
        dto.defaultVariant = getString(data, "defaultVariant", "");
        dto.labels = getStringMap(data, "labels");
    ...
    private void handlePatch(HTTPServerRequest req, HTTPServerResponse res) @safe {
        ...
        dto.state_ = getString(data, "state", "");
        dto.updatedBy = getString(data, "updatedBy", "");
