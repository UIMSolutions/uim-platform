...
struct TransportValidator
{
    ...
    static TransportValidationResult validate(
        const ref TransportRequest request,
        const ContentPackage[] packages,
        const ref TransportQueue queue)
    {
        string[] errors;
        ...
        if (request.mode == TransportMode.cloudTransportManagement && queue.queueType != QueueType.cloudTMS)
