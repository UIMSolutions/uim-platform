...
import domain.services.package_assembler;
import domain.types;
...
class ManageContentPackagesUseCase
{
    ...
    this(ContentPackageRepository packageRepo,
         ContentProviderRepository providerRepo,
    ...
    CommandResult updatePackage(ContentPackageId id, UpdatePackageRequest req)
    {
        ...
        if (req.items.length > 0) pkg.items = req.items;
    ...
    CommandResult assemblePackage(AssemblePackageRequest req)
    {
        ...
        packageRepo.update(pkg);
        recordActivity(req.tenantId, ActivityType.packageAssembled, req.packageId, pkg.name,
            "Package assembled", req.assembledBy);
    ...
    CommandResult deletePackage(ContentPackageId id)
    {
        ...
        recordActivity(pkg.tenantId, ActivityType.packageDeleted, id, pkg.name,
            "Package deleted", "");
    ...
    private static ContentFormat parseContentFormat(string s)
    {
        switch (s)
        {
            ...
            default: return ContentFormat.mtar;
        }
    }
    ...
    private static PackageStatus parsePackageStatus(string s)
    {
        switch (s)
        {
            ...
            default: return PackageStatus.draft;
        }
    }
}
