module application.use_cases.manage_transport_queues;
...
class ManageTransportQueuesUseCase
{
    ...
    CommandResult updateQueue(TransportQueueId id, UpdateQueueRequest req)
    {
        ...
        if (req.description.length > 0) queue.description = req.description;
        if (req.endpoint.length > 0) queue.endpoint = req.endpoint;
        if (req.authToken.length > 0) queue.authToken = req.authToken;
        queue.isDefault = req.isDefault;
    ...
    private static QueueType parseQueueType(string s)
    {
        switch (s)
        {
            case "ctsPlus": return QueueType.ctsPlus;
            case "local": return QueueType.local;
            default: return QueueType.cloudTMS;
        }
    }
}
