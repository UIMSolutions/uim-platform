...
class ManageTransportRequestsUseCase
{
    ...
    this(TransportRequestRepository requestRepo,
         ContentPackageRepository packageRepo,
         TransportQueueRepository queueRepo,
    ...
    CommandResult createTransportRequest(CreateTransportRequest req)
    {
        ...
        requestRepo.save(tr);
        recordActivity(req.tenantId, ActivityType.transportCreated, id, req.description,
            "Transport request created", req.createdBy);
    ...
    private static TransportMode parseTransportMode(string s)
    {
        switch (s)
        {
            case "ctsPlus": return TransportMode.ctsPlus;
            case "directExport": return TransportMode.directExport;
            case "fileDownload": return TransportMode.fileDownload;
            default: return TransportMode.cloudTransportManagement;
        }
    }
    ...
    private static TransportStatus parseTransportStatus(string s)
    {
        switch (s)
        {
            case "created": return TransportStatus.created;
            case "readyForExport": return TransportStatus.readyForExport;
            case "exporting": return TransportStatus.exporting;
            case "exported": return TransportStatus.exported;
            case "inQueue": return TransportStatus.inQueue;
            case "importing": return TransportStatus.importing;
            case "imported": return TransportStatus.imported;
            case "released": return TransportStatus.released;
            case "failed": return TransportStatus.failed;
            case "cancelled": return TransportStatus.cancelled;
            default: return TransportStatus.created;
        }
    }
}
