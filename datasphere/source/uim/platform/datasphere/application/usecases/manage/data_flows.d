...
class ManageDataFlowsUseCase {
  ...
  UsecaseResult createDataFlow(CreateDataFlowRequest r) {
    if (r.name.isEmpty)
      return UsecaseResult(false, "", "Data flow name is required");
    ...
    repo.save(df);
    return UsecaseResult(true, df.id.value, "");
  }
  ...
  UsecaseResult patchDataFlow(PatchDataFlowRequest r) {
    ...
    if (flow.isNull)
      return UsecaseResult(false, "", "Data flow not found");

    
    flow.updatedAt = currentTimestamp;

    repo.update(flow);
    return UsecaseResult(true, flow.id.value, "");
  }
