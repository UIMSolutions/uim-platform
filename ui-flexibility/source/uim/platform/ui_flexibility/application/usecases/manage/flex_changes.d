...
class ManageFlexChangesUseCase {
  ...
  CommandResult createChange(CreateFlexChangeRequest r) {
    ...
    return CommandResult(true, c.id_.value);
  }

  CommandResult updateChange(UpdateFlexChangeRequest r) {
    auto existing = repo.findById(r.tenantId, r.changeId);
    if (existing.isNull) return CommandResult(false, null, "FlexChange not found");
  ...
  FlexChange getChange(string tenantId, FlexChangeId id) {
    return repo.findById(tenantId, id);
  }

  FlexChange[] listChanges(string tenantId) {
    return repo.findByTenantAll(tenantId);
  }

  FlexChange[] listChangesByApp(string tenantId, string appId) {
    return repo.findByApp(tenantId, appId);
  }

  FlexChange[] listChangesByLayer(string tenantId, string appId, ChangeLayer layer) {
    return repo.findByLayer(tenantId, appId, layer);
  }

  CommandResult deleteChange(string tenantId, FlexChangeId id) {
    if (!repo.existsById(tenantId, id)) return CommandResult(false, null, "FlexChange not found");
    repo.removeById(tenantId, id);
    return CommandResult(true, id.value);
  }

  long countChanges(string tenantId) {
