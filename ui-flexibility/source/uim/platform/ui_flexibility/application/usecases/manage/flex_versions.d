...
class ManageFlexVersionsUseCase {
  ...
  CommandResult activateVersion(ActivateVersionRequest r) {
    ...
    if (v.isNull) return CommandResult(false, null, "FlexVersion not found");
  ...
  FlexVersion getVersion(string tenantId, FlexVersionId id) {
    return repo.findById(tenantId, id);
  }

  FlexVersion[] listVersions(string tenantId) {
    return repo.findByTenantAll(tenantId);
  }

  FlexVersion[] listVersionsByApp(string tenantId, string appId) {
    return repo.findByApp(tenantId, appId);
  }

  FlexVersion getActiveVersion(string tenantId, string appId) {
    return repo.findActiveByApp(tenantId, appId);
  }

  CommandResult deleteVersion(string tenantId, FlexVersionId id) {
    if (!repo.existsById(tenantId, id)) return CommandResult(false, null, "FlexVersion not found");
