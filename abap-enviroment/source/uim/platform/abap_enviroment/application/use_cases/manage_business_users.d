...
class ManageBusinessUsersUseCase
{
    ...
    CommandResult updateUser(BusinessUserId id, UpdateBusinessUserRequest req)
    {
        ...
        if (req.firstName.length > 0) user.firstName = req.firstName;
        if (req.lastName.length > 0) user.lastName = req.lastName;
        if (req.email.length > 0) user.email = req.email;
        if (req.status.length > 0) user.status = parseUserStatus(req.status);
...
private UserStatus parseUserStatus(string s)
{
    ...
    case "passwordLocked": return UserStatus.passwordLocked;
