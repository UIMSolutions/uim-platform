module uim.platform.abap_enviroment.application.usecases.manage_application_jobs;
...
class ManageApplicationJobsUseCase
{
    ...
    CommandResult updateJob(ApplicationJobId id, UpdateApplicationJobRequest req)
    {
        ...
        if (req.description.length > 0) job.description = req.description;
        if (req.frequency.length > 0) job.frequency = parseFrequency(req.frequency);
        if (req.scheduledAt > 0) job.scheduledAt = req.scheduledAt;
        if (req.cronExpression.length > 0) job.cronExpression = req.cronExpression;
