...
public:
  ...
  UsecaseResult create(CreatePackageRequest req) {
    ...
    return UsecaseResult(true, pkg.toJson());
  }
  ...
  UsecaseResult getById(TenantId tenantId, string id) {
    ...
    if (pkg.isNull) return UsecaseResult(false, "Package not found");
    return UsecaseResult(true, pkg.toJson());
  }

  UsecaseResult update(UpdatePackageRequest req) {
  ...
  UsecaseResult remove(TenantId tenantId, string id) {
    ...
    return UsecaseResult(true, "Package deleted");
  }
}
