...
*****************************************************************************************************************/
module uim.platform.postgres.presentation.web.views.service_plan;
...
class WebServicePlanView {
    ...
    private string buildListHtml(WebServicePlanModel model) {
        ...
        sb ~= `<table border="1"><tr><th>ID</th><th>Name</th><th>Tier</th><th>MemGB</th><th>StrgGB</th><th>Available</th></tr>`;
        foreach (p; model.plans)
            sb ~= format(`<tr><td><a href="/web/postgres/plans/%s">%s</a></td><td>%s</td><td>%s</td><td>%d</td><td>%d</td><td>%s</td></tr>`,
                p.id.value, p.id.value[0..min(8, p.id.value.length)], p.name, p.tier.to!string, p.memoryGb, p.storageGb, p.available ? "yes" : "no");
        sb ~= `</table>` ~ htmlFoot();
        return sb.data;
    }
    ...
    private string buildDetailHtml(WebServicePlanModel model) {
        ...
        sb ~= format(`<dt>ID</dt><dd>%s</dd>`, p.id.value);
        sb ~= format(`<dt>Name</dt><dd>%s</dd>`, p.name);
        sb ~= format(`<dt>Tier</dt><dd>%s</dd>`, p.tier.to!string);
        sb ~= format(`<dt>Memory GB</dt><dd>%d</dd>`, p.memoryGb);
        sb ~= format(`<dt>Storage GB</dt><dd>%d</dd>`, p.storageGb);
        sb ~= format(`<dt>Max Connections</dt><dd>%d</dd>`, p.maxConnections);
        sb ~= format(`<dt>Multi-AZ</dt><dd>%s</dd>`, p.multiAzSupported ? "yes" : "no");
        sb ~= format(`<dt>Available</dt><dd>%s</dd>`, p.available ? "yes" : "no");
        sb ~= `</dl><a href="/web/postgres/plans">Back to list</a>` ~ htmlFoot();
        return sb.data;
    }
    ...
    private string buildErrorHtml(int code, string msg) {
        return htmlHead("Error " ~ code.to!string)
            ...
            ~ `<a href="/web/postgres/plans">Back to list</a>` ~ htmlFoot();
    }
