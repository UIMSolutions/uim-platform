    ...
    , MaintenanceWindowRepository
{
    ...
    private void loadTenant(TenantId t) @trusted {
        ...
        foreach (j; arr.byValue()) {
            ...
            e.startHourUtc            = data.getLong("startHourUtc", 2);
            e.durationHours           = data.getLong("durationHours", 1);
            e.autoMinorVersionUpgrade = data.getBoolean("autoMinorVersionUpgrade", true);
            e.status                  = data.getString("status", "scheduled").to!MaintenanceStatus;
            e.lastMaintenanceAt       = data.getLong("lastMaintenanceAt", 0);
            e.nextMaintenanceAt       = data.getLong("nextMaintenanceAt", 0);
