...
*****************************************************************************************************************/
module uim.platform.alert_notification.domain.ports.usecases.events;
...
class ProduceEventsUseCase {
    protected ISubscriptionRepository subscriptions;
    protected IConditionRepository conditions;
    protected IActionRepository actions;
    protected IMatchedEventRepository matchedEvents;
    protected IUndeliveredEventRepository undeliveredEvents;
    ...
    UsecaseResult postEvent(TenantId tenantId, PostAlertEventRequest req) {
        ...
        foreach (sub; enabledSubs) {
            if (!matcher.subscriptionMatches(event, sub, allConds))
                continue;
