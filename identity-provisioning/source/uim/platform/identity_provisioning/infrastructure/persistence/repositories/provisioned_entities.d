...
class ProvisionedEntityRepository : TenantRepository!(ProvisionedEntity, ProvisionedEntityId), IProvisionedEntityRepository {
  ...
  void removeByExternalId(TenantId tenantId, string externalId, TargetSystemId targetId) {
    auto entity = findByExternalId(tenantId, externalId, targetId);
    if (entity.id != ProvisionedEntityId.init)
      remove(entity);
  }
  ...
  ProvisionedEntity[] filterBySource(ProvisionedEntity[] entities, SourceSystemId sourceId) {
    return entities.filter!(e => e.sourceSystemId == sourceId).array;
  }
...
///
unittest {
    assert(tenantRepositoryTest(new ProvisionedEntityRepository()));
}
