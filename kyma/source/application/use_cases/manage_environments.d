...
class ManageEnvironmentsUseCase
{
    ...
    CommandResult updateEnvironment(KymaEnvironmentId id, UpdateEnvironmentRequest req)
    {
        ...
        if (req.description.length > 0) env.description = req.description;
        if (req.machineCount > 0) env.machineCount = req.machineCount;
        if (req.machineType.length > 0) env.machineType = req.machineType;
        if (req.autoScalerMin > 0) env.autoScalerMin = req.autoScalerMin;
        if (req.autoScalerMax > 0) env.autoScalerMax = req.autoScalerMax;
        if (req.oidcIssuerUrl.length > 0) env.oidcIssuerUrl = req.oidcIssuerUrl;
        if (req.oidcClientId.length > 0) env.oidcClientId = req.oidcClientId;
        if (req.administrators.length > 0) env.administrators = req.administrators;
    ...
    KymaEnvironment getEnvironment(KymaEnvironmentId id) { return repo.findById(id); }
    KymaEnvironment[] listByTenant(TenantId tenantId) { return repo.findByTenant(tenantId); }
    KymaEnvironment[] listBySubaccount(TenantId tenantId, SubaccountId subId) { return repo.findBySubaccount(tenantId, subId); }
