module uim.platform.xyz.application.usecases.manage_service_bindings;

import uim.platform.xyz.application.dto;
import uim.platform.xyz.domain.entities.service_binding;
import uim.platform.xyz.domain.ports.service_binding_repository;
...
class ManageServiceBindingsUseCase
{
    ...
    CommandResult updateBinding(ServiceBindingId id, UpdateServiceBindingRequest req)
    {
        ...
        if (req.secretName.length > 0) binding.secretName = req.secretName;
        if (req.parametersJson.length > 0) binding.parametersJson = req.parametersJson;
        if (req.labels !is null) binding.labels = req.labels;
    ...
    ServiceBinding getBinding(ServiceBindingId id) { return repo.findById(id); }
    ServiceBinding[] listByNamespace(NamespaceId nsId) { return repo.findByNamespace(nsId); }
    ServiceBinding[] listByServiceInstance(ServiceInstanceId instId) { return repo.findByServiceInstance(instId); }
