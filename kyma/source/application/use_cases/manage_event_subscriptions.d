...
class ManageEventSubscriptionsUseCase
{
    ...
    CommandResult updateSubscription(EventSubscriptionId id, UpdateEventSubscriptionRequest req)
    {
        ...
        if (req.eventTypes.length > 0) sub.eventTypes = req.eventTypes;
        if (req.sinkUrl.length > 0) sub.sinkUrl = req.sinkUrl;
        if (req.sinkServiceName.length > 0) sub.sinkServiceName = req.sinkServiceName;
        if (req.sinkServicePort > 0) sub.sinkServicePort = req.sinkServicePort;
        if (req.maxInFlightMessages > 0) sub.maxInFlightMessages = req.maxInFlightMessages;
        sub.exactTypeMatching = req.exactTypeMatching;
        if (req.filterAttributes !is null) sub.filterAttributes = req.filterAttributes;
        if (req.labels !is null) sub.labels = req.labels;
    ...
    EventSubscription getSubscription(EventSubscriptionId id) { return repo.findById(id); }
    EventSubscription[] listByNamespace(NamespaceId nsId) { return repo.findByNamespace(nsId); }
    EventSubscription[] listByEnvironment(KymaEnvironmentId envId) { return repo.findByEnvironment(envId); }
    EventSubscription[] listBySource(string source) { return repo.findBySource(source); }
    ...
    private EventTypeEncoding parseTypeEncoding(string s)
    {
        switch (s)
        {
            ...
            default: return EventTypeEncoding.exact;
        }
    }
}
