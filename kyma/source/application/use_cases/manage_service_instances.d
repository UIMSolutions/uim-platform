...
class ManageServiceInstancesUseCase
{
    ...
    CommandResult updateServiceInstance(ServiceInstanceId id, UpdateServiceInstanceRequest req)
    {
        ...
        if (req.servicePlanName.length > 0) inst.servicePlanName = req.servicePlanName;
        if (req.servicePlanId.length > 0) inst.servicePlanId = req.servicePlanId;
        if (req.parametersJson.length > 0) inst.parametersJson = req.parametersJson;
        if (req.labels !is null) inst.labels = req.labels;
    ...
    ServiceInstance getServiceInstance(ServiceInstanceId id) { return repo.findById(id); }
    ServiceInstance[] listByNamespace(NamespaceId nsId) { return repo.findByNamespace(nsId); }
    ServiceInstance[] listByEnvironment(KymaEnvironmentId envId) { return repo.findByEnvironment(envId); }
