...
class ManageApplicationsUseCase
{
    ...
    CommandResult updateApplication(ApplicationId id, UpdateApplicationRequest req)
    {
        ...
        if (req.connectorUrl.length > 0) app.connectorUrl = req.connectorUrl;
        if (req.boundNamespaces.length > 0) app.boundNamespaces = req.boundNamespaces;
        if (req.labels !is null) app.labels = req.labels;
    ...
    Application getApplication(ApplicationId id) { return repo.findById(id); }
    Application[] listByEnvironment(KymaEnvironmentId envId) { return repo.findByEnvironment(envId); }
    Application[] listByTenant(TenantId tenantId) { return repo.findByTenant(tenantId); }
    ...
    private AppRegistrationType parseRegistrationType(string s)
    {
        switch (s)
        {
            ...
            default: return AppRegistrationType.apiAndEvents;
        }
    }
}
