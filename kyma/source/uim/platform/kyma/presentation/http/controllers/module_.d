...
class ModuleController : ManageHttpController {
  protected ManageModulesUseCase usecase;
...
override protected Json getHandler(HTTPServerRequest req) {
  ...
  auto tenantId = precheck.tenantId;
  auto id = precheck.id;
  auto m = usecase.getModule(tenantId, id);
  if (m.isNull)
    return errorResponse("Module not found", 404);

  return successResponse("Module retrieved successfully", "Retrieved", 200, m.toJson);
}
...
protected void handleDisable(scope HTTPServerRequest req, scope HTTPServerResponse res) {
  try {
    auto tenantId = precheck.tenantId;
    auto id = precheck.id;
    auto result = usecase.disableModule(tenantId, id);
    if (result.success)
...
override protected Json deleteHandler(HTTPServerRequest req) {
  ...
  auto tenantId = precheck.tenantId;
  auto id = precheck.id;
  auto result = usecase.deleteModule(tenantId, id);
  if (result.hasError)
    return errorResponse(result.message, 400);
