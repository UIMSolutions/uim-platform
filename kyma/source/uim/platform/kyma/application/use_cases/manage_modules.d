module uim.platform.kyma.application.usecases.manage_modules;
...
class ManageModulesUseCase
{
    ...
    CommandResult updateModule(ModuleId id, UpdateModuleRequest req)
    {
        ...
        if (req.version_.length > 0) mod.version_ = req.version_;
        if (req.channel.length > 0) mod.channel = req.channel;
        if (req.customResourcePolicy.length > 0) mod.customResourcePolicy = req.customResourcePolicy;
        if (req.configurationJson.length > 0) mod.configurationJson = req.configurationJson;
        mod.modifiedAt = clockSeconds();
    ...
    KymaModule getModule(ModuleId id) { return repo.findById(id); }
    KymaModule[] listByEnvironment(KymaEnvironmentId envId) { return repo.findByEnvironment(envId); }
    ...
    private ModuleType parseModuleType(string s)
    {
        switch (s)
        {
            ...
            case "api-gateway": return ModuleType.apiGateway;
            ...
            case "btp-operator": return ModuleType.btp_operator;
            case "keda": return ModuleType.keda;
            case "connectivity-proxy": return ModuleType.connectivityProxy;
            default: return ModuleType.custom;
        }
    }
    ...
    private string[] getKnownDependencies(ModuleType t)
    {
        switch (t)
        {
            case ModuleType.apiGateway: return ["istio"];
            case ModuleType.serverless: return [];
            case ModuleType.eventing: return ["nats"];
            case ModuleType.keda: return [];
            case ModuleType.connectivityProxy: return ["istio"];
            default: return [];
        }
    }
}
