module uim.platform.kyma.application.usecases.manage_namespaces;
...
class ManageNamespacesUseCase
{
    ...
    CommandResult updateNamespace(NamespaceId id, UpdateNamespaceRequest req)
    {
        ...
        if (req.description.length > 0) ns.description = req.description;
        if (req.cpuLimit.length > 0) ns.cpuLimit = req.cpuLimit;
        if (req.memoryLimit.length > 0) ns.memoryLimit = req.memoryLimit;
        if (req.cpuRequest.length > 0) ns.cpuRequest = req.cpuRequest;
        if (req.memoryRequest.length > 0) ns.memoryRequest = req.memoryRequest;
        if (req.podLimit > 0) ns.podLimit = req.podLimit;
        if (req.quotaEnforcement.length > 0) ns.quotaEnforcement = parseQuotaEnforcement(req.quotaEnforcement);
        ns.istioInjection = req.istioInjection;
        if (req.labels !is null) ns.labels = req.labels;
        if (req.annotations !is null) ns.annotations = req.annotations;
    ...
    Namespace getNamespace(NamespaceId id) { return repo.findById(id); }
    Namespace[] listByEnvironment(KymaEnvironmentId envId) { return repo.findByEnvironment(envId); }
    ...
    private QuotaEnforcement parseQuotaEnforcement(string s)
    {
        switch (s)
        {
            ...
            default: return QuotaEnforcement.enforce;
        }
    }
}
