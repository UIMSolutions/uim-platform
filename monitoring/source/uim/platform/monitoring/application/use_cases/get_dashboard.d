...
class GetDashboardUseCase
{
    ...
    this(MonitoredResourceRepository resourceRepo,
         AlertRepository alertRepo,
         HealthCheckRepository checkRepo,
         HealthCheckResultRepository checkResultRepo,
         MetricDefinitionRepository metricDefRepo,
        ...
        foreach (ref c; checks)
        {
            if (c.isEnabled)
            {
                ...
                if (latest.id.length > 0 &&
                    (latest.status == CheckStatus.critical || latest.status == CheckStatus.warning))
