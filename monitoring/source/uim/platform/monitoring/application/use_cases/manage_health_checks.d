...
class ManageHealthChecksUseCase
{
    ...
    CommandResult updateCheck(HealthCheckId id, UpdateHealthCheckRequest req)
    {
        ...
        if (req.intervalSeconds > 0) c.intervalSeconds = req.intervalSeconds;
        if (req.url.length > 0) c.url = req.url;
        if (req.expectedStatus.length > 0) c.expectedStatus = req.expectedStatus;
        if (req.warningThreshold != 0) c.warningThreshold = req.warningThreshold;
        if (req.criticalThreshold != 0) c.criticalThreshold = req.criticalThreshold;
        if (req.thresholdOperator.length > 0) c.thresholdOperator = parseThresholdOperator(req.thresholdOperator);
    ...
    private static CheckType parseCheckType(string s)
    {
        switch (s)
        {
            ...
            default:                return CheckType.availability;
        }
    }
    ...
    private static CheckStatus parseCheckStatus(string s)
    {
        switch (s)
        {
            ...
            default:            return CheckStatus.unknown;
        }
    }
