...
class ManageMetricsUseCase
{
    ...
    CommandResult updateDefinition(MetricDefinitionId id, UpdateMetricDefinitionRequest req)
    {
        ...
        if (req.displayName.length > 0) def.displayName = req.displayName;
        if (req.description.length > 0) def.description = req.description;
        if (req.aggregation.length > 0) def.aggregation = parseAggregation(req.aggregation);
    ...
    Metric[] queryMetrics(QueryMetricsRequest req)
    {
        if (req.startTime > 0 && req.endTime > 0)
            return metricRepo.findInTimeRange(req.tenantId, req.resourceId, req.metricName, req.startTime, req.endTime);
    ...
    MetricSummary computeSummary(TenantId tenantId, MonitoredResourceId resourceId, string metricName, long startTime, long endTime)
    {
        auto metrics = metricRepo.findInTimeRange(tenantId, resourceId, metricName, startTime, endTime);
        MetricSummary s;
        s.name = metricName;
        ...
        {
            if (m.value_ < s.minValue) s.minValue = m.value_;
            if (m.value_ > s.maxValue) s.maxValue = m.value_;
            sum += m.value_;
        }
        s.sumValue = sum;
    ...
    private static MetricCategory parseCategory(string s)
    {
        switch (s)
        {
            ...
            default:                return MetricCategory.custom;
        }
    }
    ...
    private static MetricUnit parseUnit(string s)
    {
        switch (s)
        {
            ...
            default:                return MetricUnit.none;
        }
    }
    ...
    private static AggregationMethod parseAggregation(string s)
    {
        switch (s)
        {
            ...
            default:        return AggregationMethod.average;
        }
    }
}
