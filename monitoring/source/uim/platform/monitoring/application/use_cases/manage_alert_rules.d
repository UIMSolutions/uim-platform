module uim.platform.monitoring.application.usecases.manage_alert_rules;
...
class ManageAlertRulesUseCase
{
    ...
    CommandResult createRule(CreateAlertRuleRequest req)
    {
        ...
        rule.evaluationPeriodSeconds = req.evaluationPeriodSeconds > 0 ? req.evaluationPeriodSeconds : 300;
    ...
    CommandResult updateRule(AlertRuleId id, UpdateAlertRuleRequest req)
    {
        ...
        if (req.warningThreshold != 0) rule.warningThreshold = req.warningThreshold;
        if (req.criticalThreshold != 0) rule.criticalThreshold = req.criticalThreshold;
        if (req.evaluationPeriodSeconds > 0) rule.evaluationPeriodSeconds = req.evaluationPeriodSeconds;
        if (req.consecutiveBreaches > 0) rule.consecutiveBreaches = req.consecutiveBreaches;
        if (req.severity.length > 0) rule.severity = parseSeverity(req.severity);
        rule.isEnabled = req.isEnabled;
        if (req.channelIds.length > 0) rule.channelIds = req.channelIds;
    ...
    private static ThresholdOperator parseOperator(string s)
    {
        switch (s)
        {
            ...
            default:                 return ThresholdOperator.greaterThan;
        }
    }
