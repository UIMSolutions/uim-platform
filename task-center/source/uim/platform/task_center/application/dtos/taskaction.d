...
struct PerformTaskActionRequest {
    ...
    TaskActionId taskActionId; // This is needed to ensure idempotency of the request. The client can generate a UUID and pass it here. If a task action with the same ID already exists, the service can return the existing action instead of creating a new one.
...
struct UpdateTaskActionRequest {
    ...
    TaskActionId taskActionId; // This is needed to identify which task action to update. The client must pass the ID of the action to be updated.
