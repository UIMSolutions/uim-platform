module application.use_cases.manage_lifecycle_rules;
...
class ManageLifecycleRulesUseCase
{
    ...
    CommandResult updateRule(LifecycleRuleId id, UpdateLifecycleRuleRequest req)
    {
        ...
        if (req.prefix.length > 0) rule.prefix = req.prefix;
        if (req.status.length > 0) rule.status = parseRuleStatus(req.status);
        if (req.expirationDays > 0) rule.expirationDays = req.expirationDays;
        if (req.transitionDays > 0) rule.transitionDays = req.transitionDays;
...
private RuleStatus parseRuleStatus(string s)
{
    ...
    default: return RuleStatus.enabled;
    }
}
...
private StorageClass parseStorageClass(string s)
{
    ...
    default: return StorageClass.nearline;
    }
}
