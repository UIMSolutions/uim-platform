...
class ValidationEngine
{
    ...
    private RuleViolation evaluateRule(
        ref const ValidationRule rule,
        string fieldValue,
        string[string] allFields)
    {
        RuleViolation empty;
        ...
        case RuleType.required:
            if (fieldValue.length == 0)
                return makeViolation(rule, fieldValue, "Field is required");
            break;
                ...
                try
                {
                    ...
                    if (matchFirst(fieldValue, r).empty)
                        return makeViolation(rule, fieldValue,
                            "Value does not match pattern: " ~ rule.pattern);
                }
                catch (Exception)
                {
            ...
            if (fieldValue.length > 0)
            {
                try
                {
                    ...
                    if (rule.minValue.length > 0 && val < rule.minValue.to!double)
                        return makeViolation(rule, fieldValue,
                            "Value below minimum: " ~ rule.minValue);
                    if (rule.maxValue.length > 0 && val > rule.maxValue.to!double)
                        return makeViolation(rule, fieldValue,
                            "Value above maximum: " ~ rule.maxValue);
                }
                catch (Exception)
                {
        ...
        case RuleType.enumeration:
            if (fieldValue.length > 0 && rule.allowedValues.length > 0)
            {
                ...
                foreach (av; rule.allowedValues)
                    if (av == fieldValue) { found = true; break; }
                if (!found)
                    return makeViolation(rule, fieldValue,
        ...
        case RuleType.length:
            if (fieldValue.length > 0)
            {
                try
                {
                    if (rule.minValue.length > 0
                        && fieldValue.length < rule.minValue.to!size_t)
                        return makeViolation(rule, fieldValue,
                            "Value too short, minimum length: " ~ rule.minValue);
                    if (rule.maxValue.length > 0
                        && fieldValue.length > rule.maxValue.to!size_t)
                        return makeViolation(rule, fieldValue,
    ...
    private static RuleViolation makeViolation(
        ref const ValidationRule rule, string fieldValue, string message)
    {
        RuleViolation v;
        v.ruleId = rule.id;
