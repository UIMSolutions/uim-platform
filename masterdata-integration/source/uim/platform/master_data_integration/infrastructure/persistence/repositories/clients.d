...
*****************************************************************************************************************/
module uim.platform.master_data_integration.infrastructure.persistence.repositories.clients;
...
class MemoryClientRepository : TenantRepository!(Client, ClientId), ClientRepository {
  ...
  Client[] filterByStatus(Client[] clients, ClientStatus status, size_t offset = 0, size_t limit = 0) {
    return (limit == 0)
      ? clients.filter!(e => e.status == status)
      .array.skip(offset) : clients.filter!(e => e.status == status).array.skip(offset).take(limit);
  }
  ...
  Client[] filterByType(Client[] clients, ClientType clientType, size_t offset = 0, size_t limit = 0) {
    return (limit == 0)
      ? clients.filter!(e => e.clientType == clientType)
      .array.skip(offset) : clients.filter!(e => e.clientType == clientType)
