module infrastructure.persistence.in_memory_certificate_repo;
...
class InMemoryCertificateRepository : CertificateRepository
{
    ...
    {
        return store.byValue()
            .filter!(e => e.tenantId == tenantId && e.subaccountId == subaccountId && e.certificateType == type)
            .array;
    }
    ...
    {
        return store.byValue()
            .filter!(e => e.tenantId == tenantId && e.validTo > 0 && e.validTo <= beforeTimestamp)
            .array;
    }

    void save(Certificate cert) { store[cert.id] = cert; }
    void update(Certificate cert) { store[cert.id] = cert; }
